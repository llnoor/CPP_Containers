...
template <class t> list<t>::list(size_type n) : list() {
  if (n > 0)
...
template <class T> list<T>::list(const list &l) : list() {
  for (auto i = l.begin(); i != l.end(); ++i) {
    push_back(*i);
  }
}
template <class T> list<T>::list(list &&l) { Moving(std::move(l)); }
template <class T> list<T> &list<T>::operator=(list &&l) noexcept {
  if (this != &l) {
    if (lock != nullptr) {
      clear();
      delete lock;
...
template <class T> list<T> &list<T>::operator=(const list &l) noexcept {
  if (this != &l) {
    if (lock != nullptr) {
      clear();
    }
...
template <class T> void list<T>::Moving(list &&l) {
  head = l.head;
  lock = l.lock;
  tail = l.tail;
...
template <class T> typename list<T>::size_type list<T>::size() const noexcept {
  size_type ret = 0;
...
template <class T> typename list<T>::const_reference list<T>::front() const {
  return head->data_;
}
template <class T> typename list<T>::const_reference list<T>::back() const {
  return tail->data_;
}
...
template <class T> void list<T>::getVal() {
