template <class T> stack<T>::stack() : list<T>::list() {}
template <class T> stack<T>::stack(const stack &s) : list<T>::list() {
  for (auto i = s.list<T>::head; i != s.list<T>::lock; i = i->next_) {
    push(i->data_);
  }
}
template <class T> stack<T>::stack(stack &&s) : list<T>::list(std::move(s)) {}
template <class T>
...
template <class T> bool stack<T>::empty() { return list<T>::empty(); }
template <class T> typename stack<T>::size_type stack<T>::size() {
  return list<T>::size();
}
template <class T> void stack<T>::swap(stack &other) { list<T>::swap(other); }
template <class T>
template <class... Args>
