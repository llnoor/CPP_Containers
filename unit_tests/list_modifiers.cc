...
TEST(ModifiersInsert, AddingToEndCompareInt) {
  ...
  auto s21_add = s21_test.end();
  auto std_add = std_test.end();
  s21_test.insert(s21_add, value);
  std_test.insert(--std_add, value);  // somnitelno no ok
  EXPECT_TRUE(s21_test.size() == std_test.size());
...
// TEST(ModifiersSplice, TwoFullListsCorrectStr) {
//   s21::list<int> test{1, 2, 3, 4, 5};
//   s21::list<int> source{10, 20, 30};
//   s21::list<int> result{10, 20, 30};
// }