...
class Vector {
 public:
  ...
  iterator insert(
      iterator pos,
      const_reference
          value);  // inserts elements into concrete pos and returns the
                   // iterator that points to the new element вставляет элементы
                   // в конкретный pos и возвращает итератор, указывающий на
                   // новый элемент
